...
import spacy
# Load the installed model "en_core_web_md" --try with en_trf_robertabase_lg
...
# END OF ####Nice exercise but it works only for questions in the VALID set, not in the KB --because of logic in worw 36////
...
Doc.set_extension("most_similar_questions", getter=get_most_similar_questions, force=True)

for doc in doc_validQuestions[:10]:
    print('\n', doc.text, '-', knowledgebase[knowledgebase['Context']==doc._.most_similar_questions].Utterance.values[0])
